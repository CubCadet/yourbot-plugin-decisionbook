...
@pytest.mark.parametrize("marker", [True, "1", 0, 2, {}, []])
def test_ready_rejects_every_noncanonical_schema_marker(ctx, marker):
    ctx.kv.set(SCHEMA_KEY, marker)
    with pytest.raises(StorageStateError):
        ready(ctx)


def test_ready_initializes_and_repairs_metadata(ctx):
    seed(ctx, 1)
...
def test_increment_failure_cannot_create_partial_record(ctx, monkeypatch):
    ...
    assert "Nothing was saved" in ctx.interaction.responses[-1]["content"]
...
def test_get_failure_returns_safe_view_error(ctx, monkeypatch):
    ...
    assert "could not load" in ctx.interaction.responses[-1]["content"]
...
def test_get_many_failure_returns_safe_search_error(ctx, monkeypatch):
    ...
    assert "could not search" in ctx.interaction.responses[-1]["content"]
...
def test_close_storage_failure_preserves_authoritative_open_record(ctx, monkeypatch):
    ...
    assert ctx.kv.get(decision_key(1)) == original
    assert "existing record remains authoritative" in ctx.interaction.responses[-1]["content"]


def test_secondary_count_failure_does_not_negate_primary_commit(ctx, monkeypatch):
    real_increment = ctx.kv.increment

    def fail_counter(key, amount=1, **kwargs):
        if key == OPEN_COUNT_KEY:
            raise RuntimeError("counter unavailable")
        return real_increment(key, amount=amount, **kwargs)

    monkeypatch.setattr(ctx.kv, "increment", fail_counter)
    decision_add(ctx, add_event())
    assert ctx.kv.get(decision_key(1))["id"] == 1
    assert ctx.kv.get(COUNTS_STATE_KEY) == COUNTS_DIRTY
    monkeypatch.setattr(ctx.kv, "increment", real_increment)
    assert status_counts(ctx) == (1, 0)
...
def test_sparse_keys_remain_visible_and_collision_safe(ctx):
    ...
    decision_list(ctx, interaction())
    assert "#11" in ctx.interaction.responses[-1]["embeds"][0]["description"]
...
def test_runtime_never_depends_on_list_values_cap_or_order(ctx, monkeypatch):
    ...
    assert "#101" in ctx.interaction.responses[-1]["embeds"][0]["description"]
...
def test_simultaneous_close_guard_allows_one_first_outcome(ctx, monkeypatch):
    ...
    contents = [item["content"] for item in ctx.interaction.responses if item["content"]]
